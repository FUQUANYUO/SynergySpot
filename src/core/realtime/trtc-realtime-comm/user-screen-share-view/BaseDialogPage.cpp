...
BaseDialogPage::BaseDialogPage(QWidget* parent)
    : QDialog(parent)
{
}

BaseDialogPage::~BaseDialogPage()
{
}

void BaseDialogPage::showEvent(QShowEvent* event) {
#ifdef _WIN32
    registerWndProc(reinterpret_cast<HWND>(this->winId()));
#endif // _WIN32
}

void BaseDialogPage::closeEvent(QCloseEvent* event) {
#ifdef _WIN32
    recoverWndProc(reinterpret_cast<HWND>(this->winId()));
#endif // _WIN32
}

void BaseDialogPage::changeEvent(QEvent* event) {
...
void BaseDialogPage::updateDynamicTextUI() {}

void BaseDialogPage::resetUI() {}

#ifdef _WIN32
void BaseDialogPage::registerWndProc(HWND hwnd) {
    SetWindowLong(hwnd, GWLP_WNDPROC, reinterpret_cast<LONG>(new_wndproc));
}

void BaseDialogPage::recoverWndProc(HWND hwnd) {
