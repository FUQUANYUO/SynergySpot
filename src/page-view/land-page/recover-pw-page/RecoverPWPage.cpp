...
RecoverPWPage * RecoverPWPage::instance = nullptr;
static std::mutex m;

RecoverPWPage *RecoverPWPage::getInstance() {
    if (instance == nullptr) {
        m.lock();
        if (instance == nullptr) {
            instance = new RecoverPWPage();
        }
        m.unlock();
    }
    return instance;
}
void RecoverPWPage::destroyInstance() {
    if (instance != nullptr) {
        m.lock();
        if (instance != nullptr) {
...
void RecoverPWPage::initWindow() {
    setWindowTitle(tr("RecoverPWPage"));
    ...
    _inputAccount         =  new QLineEdit(this);
    ...
    _confirmChangedButton =  new QPushButton(this);
...
void RecoverPWPage::initEdgeLayout() {
    ...
    _inputLayout->addWidget(_inputAccount);
    ...
    _inputLayout->addWidget(_inputNewPassword);
    _inputLayout->addWidget(_inputRepeatPassword);
    _inputLayout->addWidget(_confirmChangedButton);
...
void RecoverPWPage::initContent() {
    // fill scrollPicView
    ...
    _inputAccount->setPlaceholderText(tr("请输入账号"));
    _inputAccount->setAlignment(Qt::AlignCenter);
    _inputNewPassword->setPlaceholderText(tr("请输入新密码"));
    _inputNewPassword->setAlignment(Qt::AlignCenter);
    _inputRepeatPassword->setPlaceholderText(tr("请再次确认密码"));
    _inputRepeatPassword->setAlignment(Qt::AlignCenter);
    _inputEmail->setPlaceholderText(tr("请输入账号绑定的邮箱"));
    ...
    int height = 35;
    int width = 260;
    _inputAccount->setFixedSize(width,height);
    _inputAccount->setStyleSheet(inputLineStyle);
    _inputNewPassword->setFixedSize(width,height);
    _inputNewPassword->setStyleSheet(inputLineStyle);
    _inputNewPassword->setEchoMode(QLineEdit::Password);
    ...
    _confirmChangedButton->setText("确认修改密码");
    _confirmChangedButton->setStyleSheet("border: none;background: rgb(0,102,204);color: rgb(255,255,255);border-radius: 10px;");
    _confirmChangedButton->setFixedSize(260,40);
    ...
    _sendEmailCodeButton->setGeometry(_inputEmailCode->pos().x()+210,_inputNewPassword->pos().y() + 7,50,30);
...
void RecoverPWPage::initConnectFunc() {
    // update the bk frame
    connect(_timer, &QTimer::timeout, this, &RecoverPWPage::sltUpdateFrame);

    // send recover pw to end back
    connect(_confirmChangedButton,&QPushButton::clicked,this,&RecoverPWPage::sltCheckInfo);
    ...
    connect(_sendEmailCodeButton, &QPushButton::clicked, this, [=]() {
        ...
        do {
            ...
            emit sigEmailCodeRequest(_inputAccount->text(),_inputEmail->text());
            _loadBar->show();
        }while (false);
    });

    // get response of email code request
    connect(this, &RecoverPWPage::sigEmailCodeResponse, this, [=](const QString& requestSSIDByDTO, const QString & rightEmailCode) {
        if(_inputAccount->text() != requestSSIDByDTO) {
            LOG_ERROR("recover ssid not same with inputAccount->text()! revise not allow to execute")
            return ;
        }
...
void RecoverPWPage::setAcrylicForBKMaterial(bool enable) {
    _enableAcrylic = enable;
}

bool RecoverPWPage::generateNumberVerifyCode() {
    // 生成5位随机验证码
...
void RecoverPWPage::paintEvent(QPaintEvent *event) {
...
void RecoverPWPage::sltUpdateFrame(){
    auto * curPlugin = g_pPluginManager->getCurrentPlugin(BK_PLUGIN_NAME);
    if(curPlugin && std::string(curPlugin->getPluginName()) == BK_PLUGIN_NAME){
...
void RecoverPWPage::sltCheckInfo(){
    // consequence check the lien edit is empty
    std::regex inputRegex("^[a-zA-Z0-9_]+$");
    std::regex emailRegex("^[a-z0-9A-Z]+[- | a-z0-9A-Z . _]+@([a-z0-9A-Z]+(-[a-z0-9A-Z]+)?\\.)+[a-z]{2,}$");
    do {
        // ssid check
        if (_inputAccount->text().isEmpty()) {
            ElaMessageBar::error(ElaMessageBarType::BottomRight,"错误", "🥰请输入待找回密码的账号🥰",_displayTime, this);
            break;
        }
        ...
        // pd check
        if (!_inputNewPassword->text().isEmpty()) {
            if (!std::regex_match(_inputNewPassword->text().toStdString(),inputRegex)&& _inputNewPassword->text().length() > 30) {
        ...
        if (!_inputRepeatPassword->text().isEmpty()) {
            if (_inputRepeatPassword->text() != _inputNewPassword->text()) {
        ...
        if (!_verifyEmail)
            break;
        _verifyEmail = false;
        std::string ssid = _inputAccount->text().toStdString();
        std::string password = _inputNewPassword->text().toStdString();

        emit sigRecoverPasswordRequest({ssid,password});
        ElaMessageBar::success(ElaMessageBarType::Top,"🎉成功", "😁请稍后重写使用新密码登录吧！",_displayTime, this);
    }while (false);
}